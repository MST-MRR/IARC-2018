...
# length of dashed line divider
DASHED_LINE_LENGTH = 50
...
def optimize(func):
    ...
    def decorate(param_space, *args, **kwargs):
        ...
        **kwargs: dict, optional
        ...
        tuner = lambda params, trials=trials, num_trials=num_evals: func(params, trials, num_trials, *args, **kwargs)
        best = fmin(tuner, param_space, algo=tpe.suggest, max_evals=num_evals, trials=trials)
        return best, trials
...
@optimize
def tune(params, trials, num_trials, model, dataset_manager, labels, metric, num_folds=DEFAULT_NUM_FOLDS, num_epochs=DEFAULT_NUM_EPOCHS, verbose=True):
    """
    ...
    trials: hyperopt.Trials
        A Trials object which contains information about the entire hyperparameter tuning run.
    num_trials: int
        Number of distinct parameter sets to evaluate.
    ...
    with ClassifierDataset(paths[0], paths[1], labels) as dataset:
        ...
        for i, (X_train, X_test, y_train, y_test) in enumerate(dataset.stratified_splitter(num_folds)):
            ...
            if not i:
                log('-' * DASHED_LINE_LENGTH)
                log('Trial %d/%d' % (len(trials), num_trials))
                log('-' * DASHED_LINE_LENGTH)
